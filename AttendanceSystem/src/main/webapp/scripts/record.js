...
function paging(name,dept,start,end,currPage){
	console.log(name+" "+dept+" "+currPage);
	$.ajax({
		url:path+"/record/otherPage.io",
		type:"post",
		dataType:"json",
		data:{"empName":name,"empDept":dept,"start":start,"end":end,"page":currPage},
		success:function(result){
			deleteDataGrid();
			if(result.state==1){
				var data=result.data
...
function dataImport(){
	...
	//判断检索条件
	if($("#start").val().trim()!="" && $("#end").val().trim()!="" && end>=start){
		location.href=path+"/record/downloadExcel.io?name="+name+"&dept="+dept+"&start="+start+"&end="+end;
	}	
}